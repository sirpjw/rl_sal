...
{
#ifdef USE_TORCH
    // Use single thread to avoid conflicts with MuJoCo physics thread
...
std::vector<float> TorchModel::forward(const std::vector<std::vector<float>>& inputs)
{
    ...
    try
    {
        ...
        {
            auto tensor = torch::tensor(input, torch::kFloat32).reshape({1, static_cast<int64_t>(input.size())});
    ...
    const std::vector<std::vector<int64_t>>& shapes)
{
    ...
    {
        throw std::runtime_error("Input count and shape count do not match");
    }
    ...
    try
    {
        ...
        for (size_t i = 0; i < inputs.size(); ++i)
        {
            ...
            if (expected_size != static_cast<int64_t>(inputs[i].size()))
            {
                throw std::runtime_error("Input data size does not match requested tensor shape");
            }
            input_tensors.push_back(vector_to_torch(inputs[i], shapes[i]));
        }
